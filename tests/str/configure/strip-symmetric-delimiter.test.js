module.exports = ({ test, assert }) => ({ str }) => {
    ...
    test('removes quotes with default config', () => {
        const unquote = str.configure.stripSymmetricDelimiter(); // replace with actual exported name
    ...
    test('returns unchanged if no surrounding quotes', () => {
        const unquote = str.configure.stripSymmetricDelimiter();
        const actual = unquote('hello');
        const expected = 'hello';
        assert.equal(actual, expected);
    });
    ...
    test('handles custom delimiter', () => {
        const unquote = str.configure.stripSymmetricDelimiter({ delimiter: "'" });
    ...
    test('removes escaped quotes when allowEscaped = true (config)', () => {
        const unquote = str.configure.stripSymmetricDelimiter({ allowEscaped: true });
